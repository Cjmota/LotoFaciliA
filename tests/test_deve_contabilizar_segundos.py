...
def test_deve_contabilizar_segundos():
    ...
    ranking_mae = BenchmarkRanking([
        ...
        BenchmarkScore("Markov", "mae", "media", 0.05),

        BenchmarkScore("Bayes", "mae", "media", 0.08),

        BenchmarkScore("IA", "mae", "media", 0.09),
    ...
    assert tabela.obter("Bayes").segundos == 1
    assert tabela.obter("Markov").segundos == 1
    assert tabela.obter("IA").segundos == 0